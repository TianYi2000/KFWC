...
from net.three_stream import BaseLineNet
import cv2
...
def train(model, train_loader, optimizer, scheduler, criterion, epoch, log):
    ...
    for batch_idx, (fundus, OCT, id, mask, type, target) in enumerate(tbar):
        fundus, OCT,  id, mask, type, target = fundus.cuda(), OCT.cuda(),  id.cuda(), mask.cuda(), type.cuda(), target.cuda()
        # target = torch.tensor(target, dtype=torch.long).clone().detach()
        target = target.long()
        optimizer.zero_grad()
        output = model(fundus, OCT, id, mask, type)
...
def main():
    model = BaseLineNet(fundus_model=args.fundus_model,
                         OCT_model=args.oct_model , num_classes=classCount)
    ...
    train_loader = torch.utils.data.DataLoader(
        MultiModeDataset(data_dir, 'train', train_fundus_tf, train_OCT_tf, classCount, list_dir=list_dir),
        batch_size=args.batch_size, shuffle=True,
        num_workers=args.workers, pin_memory=True, drop_last=False
    )
    val_loader = torch.utils.data.DataLoader(
        MultiModeDataset(data_dir, 'val', val_fundus_tf, val_OCT_tf, classCount, list_dir=list_dir),
    ...
    train_log = open('logs/base_three/'+ model_name + '-train.log', 'w')
    val_log = open('logs/base_three/'+ model_name + '-val.log', 'w')
    max_avg = 0
    ...
    for epoch in range(0, args.epoch):
        ...
        if avg > max_avg:
            torch.save(model, './model/base_three/' + model_name + '.pth')
            max_avg = avg
...
if __name__ == '__main__':
    ...
    print("Train Base Three ", model_name)
    start = time.time()
    main()
    end = time.time()
    print('Finish Base Three, Time=', end - start)
