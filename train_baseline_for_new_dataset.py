...
def main():
    ...
    train_log = open('logs/baseline/'+ model_name + '-train.log', 'w')
    val_log = open('logs/baseline/'+ model_name + '-val.log', 'w')
    max_avg = 0
    ...
    for epoch in range(0, args.epoch):
        ...
        if avg > max_avg:
            torch.save(model, './model/baseline/' + model_name + '.pth')
            max_avg = avg


if __name__ == '__main__':
    args = get_parser()
    NAME = str(args.epoch) + "+" + str(args.learning_rate) + '+' + str(args.weight_decay) + '+' + args.loss + '+new-dataset'
