...
def get_parser():
    ...
    parser.add_argument('--fundus_path', type=str, help='the model file path of fundus model' , required=True)
    parser.add_argument('--oct_path', type=str, help='the model file path of OCT model' , required=True)
...
def main():
    model = TwoStreamNet(fundus_path=args.fundus_path, OCT_path=args.oct_path, fundus_model=args.fundus_model,
    ...
    train_log = open('logs/two_stream/'+ model_name + '-train.log', 'w')
    val_log = open('logs/two_stream/'+ model_name + '-val.log', 'w')
    max_avg = 0
    ...
    for epoch in range(0, args.epoch):
        ...
        if avg > max_avg:
            torch.save(model, './model/two_stream/' + model_name + '.pth')
            max_avg = avg
...
if __name__ == '__main__':
    ...
    print("Train Two stream ", model_name)
    start = time.time()
    main()
    end = time.time()
