...
fundus_path = './model/fundus/2021_05_19+resnet18++500+0.001+0.0001+bceloss.pth'
OCT_path = './model/OCT/2021_05_18+resnet18++500+0.001+0.0001+bceloss.pth'
model_name = '2021_05_26+' + FUNDUS_MODEL + '+' + OCT_MODEL + '+' + NAME + '.pth'

print("Train two_stream ", model_name, 'RESUME:', RESUME)
...
def main():
    model = TwoStreamNet(fundus_path=fundus_path, OCT_path=OCT_path, fundus_model=FUNDUS_MODEL, OCT_model=OCT_MODEL,
    ...
    writer = SummaryWriter(os.path.join('runs', 'two_stream_' + model_name[:-4]))
    for epoch in range(START_EPOCH, EPOCHS):
        ...
        if avg > max_avg:
            torch.save(model, './model/two_stream/' + model_name)
            max_avg = avg
    writer.close()
