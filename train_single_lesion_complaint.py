...
from net.three_stream import Single_Lesion_Complaint_Net
import cv2
...
def train(model, train_loader, optimizer, scheduler, criterion, epoch, log):
    ...
    for batch_idx, (image, lesion_id, lesion_mask, lesion_type, complaint_id, complaint_mask, complaint_type, target) in enumerate(tbar):
        ...
        target = target.long()
        optimizer.zero_grad()
        output = model(image, lesion_id, lesion_mask, lesion_type, complaint_id, complaint_mask, complaint_type)
...
def main():
    model = Single_Lesion_Complaint_Net(image_model=args.oct_model, lock_text_weight = args.lock_text_weight)
    ...
    train_log = open('logs/single_lesion_complaint/'+ model_name + '-train.log', 'w')
    val_log = open('logs/single_lesion_complaint/'+ model_name + '-val.log', 'w')
    max_avg = 0
    ...
    for epoch in range(0, args.epoch):
        ...
        if avg > max_avg:
            torch.save(model, './model/single_lesion_complaint/' + model_name + '.pth')
            max_avg = avg
...
if __name__ == '__main__':
    ...
    print("Train Single Lesion Complaint Net ", model_name)
    start = time.time()
    main()
    end = time.time()
    print('Finish Single Lesion Complaint Net, Time=', end - start)
